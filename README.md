...
- The are three main files to consider:
  - _transformer_pipeline.ipynb_, which is a notebook used to train the transformer based agent,         simple change the variable in the if '__name__' == '__main__' section to the desired               cofiguration and run the cell. make sure the train argument on the main method is set to           True
  ...
  - _PlotLearnigCurve.ipynb, which is used to extract trainig related data for evaluation and           creating learnign curves, simply run cells from first going down, works post-training as it        extract data store in tensorboards       
